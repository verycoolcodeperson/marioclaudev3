...
export class Boss {
  ...
  // Bosses are fought in phases of N hits rather than a raw HP pool; these
  // flatten that into one continuous 0..max value so the HUD can draw a
  // single bar that drains smoothly across every phase.
  get maxHealth() { return this.maxPhase * this.hitsPerPhase; }
  get health() {
    if (this.defeated) return 0;
    const landed = (this.phase - 1) * this.hitsPerPhase + this.hitsTaken;
    return Math.max(0, this.maxHealth - landed);
  }
