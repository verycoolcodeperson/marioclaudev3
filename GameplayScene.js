...
export class GameplayScene {
  async init(payload) {
    ...
    if (this.levelSpec.cameraOffset) this.cameraRig.setOffset(this.levelSpec.cameraOffset);

    const start = this.levelSpec.playerStart;
    this.player1 = new Player({ character: SaveSystem.getCharacter('p1'), playerId: 'p1', x: start.x, y: start.y, z: start.z });
    ...
    if (this.multiplayer) {
      ...
      this.player2 = new Player({ character: SaveSystem.getCharacter('p2'), playerId: 'p2', x: start.x, y: start.y, z: start.z });
  ...
  _wirePlayer(p) {
    ...
    p.onShootFireball = (x, y, z, dirX, dirZ) => {
      this.level.projectiles.spawn({
        x, y, z,
        vx: dirX * 13, vy: 2.4, vz: dirZ * 13,
        type: 'fireball', gravity: -18, life: 1.6, damage: 1,
        owner: p.playerId, radius: 0.34,
      });
    };
  }

  /** Player fireballs vs. enemies and bosses (enemy shots are handled in ProjectileSystem). */
  _checkPlayerProjectiles() {
    this.level.projectiles.forEachActive((proj) => {
      if (proj.owner === 'enemy') return false;
      for (const enemy of this.level.enemies) {
        if (!enemy.alive) continue;
        if (distance2D(proj.mesh.position.x, proj.mesh.position.z, enemy.position.x, enemy.position.z) < proj.radius + enemy.radius
          && Math.abs(proj.mesh.position.y - (enemy.position.y + (enemy.bodyHeight || 0.8) * 0.5)) < 1.3) {
          enemy.takeHit(false);
          this.particles.burst(enemy.position.x, enemy.position.y + 0.4, enemy.position.z, 0xff7a2f, 8, 3);
          const owner = this.players.find((pl) => pl && pl.playerId === proj.owner);
          if (enemy.config.explodes && !enemy.alive && owner) this._explode(enemy, owner);
          return true;
        }
      }
      const boss = this.level.boss;
      if (boss && boss.vulnerable && !boss.defeated
        && distance2D(proj.mesh.position.x, proj.mesh.position.z, boss.position.x, boss.position.z) < proj.radius + boss.radius) {
        boss.takeHit();
        this.particles.burst(boss.position.x, boss.position.y + 1, boss.position.z, 0xff7a2f, 10, 3.4);
        return true;
      }
      return false;
    });
  ...
  /** Head-bumping a brick: Mega players smash it, everyone else just knocks it. */
  _checkBricks() {
    for (const brick of this.level.bricks) {
      if (brick.broken) continue;
      const col = brick.collider;
      for (const player of this.players) {
        if (!player || player.dead || player.velocity.y <= 0) continue;
        const r = player.bodyRadius;
        const withinX = player.position.x > col.minX - r && player.position.x < col.maxX + r;
        const withinZ = player.position.z > col.minZ - r && player.position.z < col.maxZ + r;
        const headY = player.position.y + player.bodyHeight;
        if (withinX && withinZ && headY > col.minY - 0.15 && headY < col.minY + 0.45) {
          if (brick.hit(player.isMega)) {
            this.particles.burst(col.x, col.y, col.z, 0xb5651d, 12, 3.5);
          } else {
  ...
  _checkPowerups() {
    for (const pw of this.level.powerups) {
      ...
      for (const player of this.players) {
        ...
        if (pw.tryCollect(player.position.x, player.position.z, player.position.y)) {
          player.applyPowerup(pw.kind);
          this.particles.burst(pw.position.x, pw.position.y + 0.3, pw.position.z,
            pw.kind === 'fire' ? 0xff7a2f : 0xe6302c, 14, 3.2);
          break;
        }
      }
    }
  ...
  _checkEnemyCollisions(dt) {
    for (const enemy of this.level.enemies) {
      ...
      for (const player of this.players) {
        ...
        if (d < enemy.radius + player.bodyRadius) {
  ...
  _checkBossCollisions() {
    ...
    for (const player of this.players) {
      ...
      if (d < boss.radius + player.bodyRadius) {
  ...
  _checkBlocks() {
    for (const block of this.level.blocks) {
      ...
      for (const player of this.players) {
        if (!player || player.dead || player.velocity.y <= 0) continue;
        const withinX = player.position.x > col.minX - player.bodyRadius && player.position.x < col.maxX + player.bodyRadius;
        const withinZ = player.position.z > col.minZ - player.bodyRadius && player.position.z < col.maxZ + player.bodyRadius;
        const headY = player.position.y + player.bodyHeight;
  ...
  update(dt) {
    ...
    this._updateCameraAndParticles(dt);
    this.hud.update(this.players, this.level);
    this.hud.setBoss(this.level.boss);
