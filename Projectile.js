...
export class ProjectileSystem {
  ...
  /** Walks every live projectile. `cb(p)` returning true despawns it. */
  forEachActive(cb) {
    for (const p of this.pool) {
      if (!p.active) continue;
      if (cb(p)) { p.active = false; p.mesh.visible = false; }
    }
  }
  ...
  update(dt, players, onHitPlayer) {
    for (const p of this.pool) {
      ...
      for (const player of players) {
        ...
        if (d2 < p.radius + player.bodyRadius && Math.abs(p.mesh.position.y - (player.position.y + 0.8)) < 1.2) {
