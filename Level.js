...
import { createPowerup, createBrickBlock } from '../entities/Powerup.js';
...
import { brickTexture } from '../utils/textures.js';
...
export class Level {
  ...
  _buildBricks() {
    for (const b of this.spec.bricks || []) {
      const brick = createBrickBlock(b, brickTexture());
      this.scene.add(brick.mesh);
      this.bricks.push(brick);
      this.colliders.push(brick.collider);
      this.collisionMeshes.push(brick.mesh);
    }
  }

  _buildPowerups() {
    for (const w of this.spec.powerups || []) {
      const pw = createPowerup(w);
      this.scene.add(pw.mesh);
      this.powerups.push(pw);
    }
  }

  /** Spawns a powerup popped from a bumped Lucky Block. */
  dispensePowerup(x, y, z, kind) {
    const pw = createPowerup({ x, y: y + 0.4, z, kind, id: `blockpw_${x}_${z}` });
    this.scene.add(pw.mesh);
    this.powerups.push(pw);
  }
  ...
  dispenseFromBlock(x, y, z, contents) {
    if (contents === 'fire' || contents === 'mega') {
      this.dispensePowerup(x, y, z, contents);
  ...
  update(dt, players, particleSystem) {
    ...
    for (const pw of this.powerups) pw.update(dt, this.time);
