...
export class WorldMapScene {
  async init({ mode = 'hub', world = 1, multiplayerIntent = false, cameFrom = null } = {}) {
    ...
    this.player = new Player({ character: SaveSystem.getCharacter('p1'), playerId: 'p1', x: start.x, y: start.y, z: start.z });
  ...
  _buildEnvironment() {
    ...
    // The visible ground runs far wider than the walkable area and sinks a
    // long way down, so its edges dissolve into the fog instead of ending in
    // mid-air. Previously this was a 46-wide slab with visible sides, which
    // read as a floating island hanging in the sky.
    const visualSize = groundSize * 2.8;
    const ground = createPlatform({ x: 0, y: -20, z: 0, sx: visualSize, sy: 40, sz: visualSize, color: this.theme.ground, textureType: 'grass' });
    ...
    // A dense treeline planted along those invisible walls, so the edge of
    // the walkable area reads as a forest boundary rather than the player
    // bumping into nothing.
    const ringR = half - 0.5;
    const ringCount = this.mode === 'hub' ? 46 : 32;
    for (let i = 0; i < ringCount; i++) {
      const a = (i / ringCount) * Math.PI * 2;
      const jitter = 0.85 + Math.random() * 0.5;
      const tx = Math.cos(a) * (ringR + Math.random() * 2.5);
      const tz = Math.sin(a) * (ringR + Math.random() * 2.5);
      const tree = createTree(tx, 0, tz, {
        leafColor: this.theme.accent, trunkColor: 0x6b4a2b, scale: 1.25 * jitter,
      });
