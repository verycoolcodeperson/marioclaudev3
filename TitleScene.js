...
import { SaveSystem, PLAYABLE_CHARACTERS } from '../core/SaveSystem.js';
import { showControlsPanel } from '../ui/ControlsPanel.js';
...
function injectStyle() {
  ...
  style.textContent = `
    ...
    .char-label { color:#8fd6ff; font-size:12px; font-weight:800; letter-spacing:2px; text-align:left; margin:12px 0 6px; }
    .char-grid { display:grid; grid-template-columns:repeat(4,1fr); gap:8px; }
    .char-btn { display:flex; flex-direction:column; align-items:center; gap:3px; padding:9px 5px; border-radius:12px; border:3px solid rgba(255,255,255,0.25); background:rgba(255,255,255,0.08); cursor:pointer; transition:transform 0.08s; }
    .char-btn:hover { transform:translateY(-2px); }
    .char-btn.active { border-color:#ffe066; background:rgba(255,224,102,0.22); }
    .char-swatch { width:26px; height:26px; border-radius:50%; border:2px solid rgba(0,0,0,0.35); }
    .char-mario { background:#e6302c; } .char-luigi { background:#35a54a; }
    .char-peach { background:#ff8fc7; } .char-daisy { background:#ff9d2e; }
    .char-name { color:#fff; font-size:12px; font-weight:800; }
    .char-blurb { color:rgba(255,255,255,0.6); font-size:9px; line-height:1.2; text-align:center; }
  `;
...
export class TitleScene {
  ...
  _buildDOM() {
    ...
    this.overlay.innerHTML = `
      <div class="title-card">
        ...
        <div class="title-menu">
          ...
          <button class="title-btn secondary" data-a="characters">CHARACTERS</button>
          <button class="title-btn secondary" data-a="controls">CONTROLS</button>
    ...
    this.overlay.querySelector('[data-a="characters"]').onclick = () => this._showCharacters();
    this.overlay.querySelector('[data-a="controls"]').onclick = () => showControlsPanel(APP);
  ...
  /** Pick who Player 1 and Player 2 play as. Saved, so it sticks between runs. */
  _showCharacters() {
    SFX.unlock();
    SFX.play('select');
    const panel = document.createElement('div');
    panel.className = 'title-panel';
    const grid = (pid) => PLAYABLE_CHARACTERS.map((c) => `
      <button class="char-btn${SaveSystem.getCharacter(pid) === c.key ? ' active' : ''}" data-pid="${pid}" data-key="${c.key}">
        <span class="char-swatch char-${c.key}"></span>
        <span class="char-name">${c.name}</span>
        <span class="char-blurb">${c.blurb}</span>
      </button>`).join('');
    panel.innerHTML = `
      <div class="title-panel-card">
        <div class="title-panel-title">CHOOSE YOUR CHARACTER</div>
        <div class="char-label">PLAYER 1</div>
        <div class="char-grid" data-grid="p1">${grid('p1')}</div>
        <div class="char-label">PLAYER 2</div>
        <div class="char-grid" data-grid="p2">${grid('p2')}</div>
        <button class="title-panel-btn" data-a="close">Done</button>
      </div>`;
    APP.appendChild(panel);
    panel.querySelectorAll('.char-btn').forEach((btn) => {
      btn.onclick = () => {
        const { pid, key } = btn.dataset;
        SaveSystem.setCharacter(pid, key);
        SFX.play('select');
        panel.querySelectorAll(`[data-grid="${pid}"] .char-btn`).forEach((b) => b.classList.toggle('active', b === btn));
      };
    });
